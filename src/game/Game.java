...
public class Game {
	...
	public void setScore(int s) {
		...
		while (pit.hasNext()) {
			Player pl = pit.next();
			int[] ints = new int[] { -5, 0, score, 0, 0, 0, 0, 0, 0, 0 };
	...
	public Game(KeyboardInput keyboard, MouseInput mouse) {
		...
		/*
		 * Player p = new Player(this, keyboard, mouse); p.viewport = new
		 * Viewport(this, p); players.add(p);
		 * 
		 * 
		 * 
		 * server = new Server();
	...
	public void update(double delta) {
		...
		if (!players.isEmpty()) {
			if (timeUntilSpawn <= 0) {
				...
				spawnMonster(new DefaultMonster(this,
						new Point2D.Double(rand.nextDouble() * roomW, rand.nextDouble() * roomH)));
				if (monsters.size() % 4 == 0) {
					spawnMonster(new Monster2(this,
							new Point2D.Double(rand.nextDouble() * roomW, rand.nextDouble() * roomH)));
				}
			} else
	...
	public void render(Graphics2D g) {
		Iterator<Player> pit = players.iterator();
		while (pit.hasNext()) {
			drawOnViewport(g, pit.next().viewport);
		}
	}
	...
	public void drawOnViewport(Graphics2D g, Viewport viewport) {
		...
		// g.fillRect(roomCoord.x, roomCoord.y, (int)(roomW*viewport.ppu),
		// (int)(roomH*viewport.ppu));

		//viewport.drawSprite(new Rectangle.Double(0, 0, roomW, roomH), arena, g);

		Iterator<Player> pit = players.iterator();
		while (pit.hasNext())
			pit.next().draw(g, viewport);
		ListIterator<Monster> mit = monsters.listIterator(0);
		while (mit.hasNext())
			mit.next().draw(g, viewport);
		ListIterator<Entity> eit = entities.listIterator(0);
		while (eit.hasNext())
			eit.next().draw(g, viewport);
